...
const RegularTicketAccordionItem = ({ ticketData, index }) => {
    const itemRef = createRef()

    let ticket = decodeJwtData(ticketData);

    let issueTime = "";
    if (ticketData != undefined)
        issueTime = new Date(ticket?.iat * 1000).toString();
    ...
    return (
        <div className="accordion-item">
            <div className="accordion-header">
                <button className="accordion-button fw-bold collapsed" type="button" data-bs-toggle="collapse" data-bs-target={`#card${index}`} aria-expanded="true" aria-controls={`card${index}`}>
                    {ticket?.sourceStationName} ({ticket?.sourceStationCode}) - {ticket?.destinationStationName} ({ticket?.destinationStationCode})
                </button>

                <small>Booked on : {issueTime}</small>
            </div>

            <div id={`card${index}`} className="accordion-collapse collapse" data-bs-parent="#accordionExample">
                <div className="accordion-body py-2">
                    <p className='my-0'>From : <strong>{ticket?.sourceStationName} ({ticket?.sourceStationCode})</strong></p>
                    <p className='my-0'>To :  <strong>{ticket?.destinationStationName} ({ticket?.destinationStationCode})</strong></p>
                    <p className='my-0'>Passenger : <strong>{ticket?.numberOfPassenger} </strong></p>
                    ...
                    <div className="optionsBox d-flex flex-row w-100 align-items-center justify-content-evenly py-2">
                        <button onClick={() => shareTicket(ticketData)} type="button" className="btn btn-primary">
                        ...
                        <button onClick={() => downloadTicket(ticketData)} type="button" className="btn btn-success">
